...
def made_urls_before(date: str) -> set[str]:
    """예전에 만든 세트의 원문 주소(docs/날짜/set-*/set.json 에서 읽음)."""
    from .common import docs_dir
    urls: set[str] = set()
    for p in docs_dir().glob("20*-*-*/set-*/set.json"):
        if p.parent.parent.name == date:
            continue
        urls.update(read_json(p, {}).get("source_urls", []))
    return urls
